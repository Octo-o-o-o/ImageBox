...
export function Sidebar() {
  ...
  return (
    <div className={clsx(
      ...
      {/* Header */}
      <div className={clsx(
        "flex items-center gap-2 mb-8 relative z-10 titlebar-no-drag transition-all",
        isCollapsed ? "px-0 justify-center" : "px-2"
      )}>
        {!isCollapsed && (
          <>
            <img src="/logo.png" alt="ImageBox" className="w-10 h-10 rounded-lg shadow-lg" />
            <span className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-orange-500 to-orange-600 dark:from-white dark:to-white/70">
              ImageBox
            </span>
          </>
        )}
        {isCollapsed && (
          <img src="/logo.png" alt="ImageBox" className="w-8 h-8 rounded-lg shadow-lg" />
        )}
      </div>
      ...
      <div className={clsx("mt-auto flex flex-col gap-2", isCollapsed && "items-center")}>
        ...
        {isCollapsed && (
          <button
            ...
            className="p-2.5 rounded-lg text-muted-foreground hover:bg-secondary/50 hover:text-foreground transition-colors"
        ...
        <a
            ...
            className={clsx(
              "flex items-center gap-3 rounded-lg text-muted-foreground hover:bg-secondary/50 hover:text-foreground transition-all group",
              isCollapsed ? "px-2 py-2.5 justify-center" : "px-3 py-2.5"
            )}
            title={isCollapsed ? t('common.github') : undefined}
        >
            <Github className="w-5 h-5 shrink-0" />
            {!isCollapsed && <span className="font-medium text-sm group-hover:underline decoration-border underline-offset-4">{t('common.github')}</span>}
        </a>
        ...
        <button
            ...
            className={clsx(
              "flex items-center gap-3 rounded-lg text-muted-foreground hover:bg-secondary/50 hover:text-foreground transition-all",
              isCollapsed ? "px-2 py-2.5 justify-center" : "px-3 py-2.5"
            )}
        ...
        >
            {isCollapsed ? (
              ...
              <>
                ...
                <span className="font-medium text-sm">{t('sidebar.collapse')}</span>
              </>
            )}
        </button>
