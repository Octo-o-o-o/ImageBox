...
import { getTemplates, generateImageAction, generateTextAction, getModels, getFolders, ensureDefaultFolder, getStorageConfig } from '@/app/actions';
            ...
            <div className="max-h-64 overflow-y-auto custom-scrollbar">
              ...
              {sortedModels.map(m => {
                ...
                return (
                  <div
                    key={m.id}
                    className={`w-full px-4 py-2.5 text-left text-sm flex items-center justify-between gap-3 transition-colors ${needsApiKey
...
function StudioPageContent() {
  ...
  // Update global generation state when local state changes
  useEffect(() => {
    setGlobalGenerating(generatingImage);
  }, [generatingImage, setGlobalGenerating]);

  useEffect(() => {
    setIsDesktopMode(isDesktopApp());
    // 判断是否自定义存储路径（影响 URL 生成策略）
    getStorageConfig()
  ...
  useEffect(() => {
    const handleConfigChanged = () => {
      ...
      getModels().then(freshModels => {
        ...
        const imageModels = freshModels.filter((m: any) => m.type === 'IMAGE');
        const firstAvailableModel = imageModels.find((m: any) => {
          const needsApiKey = m.provider && m.provider.type !== 'LOCAL' && !m.provider.apiKey;
          return !needsApiKey;
        });
    ...
    return () => window.removeEventListener('imagebox:config-changed', handleConfigChanged);
  }, []);
  ...
  const performImageGeneration = async () => {
    ...
    try {
      ...
      if (res.success && res.images) {
        // 后端返回的 url 可能是 /generated/...；桌面模式需要转换为 /api/images/... 以读取 USER_DATA_PATH
        const newImages = (res.images as { id: string, url: string, prompt: string }[]).map((img) => {
          // 后端通常返回 /generated/...；若已经是 /api/images/... 则不重复转换
          const url = img.url.startsWith('/api/images/')
            ? img.url
            : getImageUrl(img.url, isCustomStoragePath, isDesktopMode);
          return { ...img, url };
        });
      ...
      <div className="w-[280px] flex-none flex flex-col gap-4 overflow-y-auto">
        <div className="flex flex-col gap-5 p-5 rounded-2xl bg-card border border-border backdrop-blur-md min-h-full shadow-sm">
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-bold text-muted-foreground/70 uppercase tracking-widest">{tr('create.imageModelLabel')}</label>
            <ImageModelDropdown
              models={imageModels}
          ...
          {supportedParams.includes('aspectRatio') && (
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-muted-foreground/70 uppercase tracking-widest">{tr('create.aspectRatioLabel')}</label>

              <button
                onClick={() => setAspectRatio(ASPECT_RATIO_ORIGINAL)}
                className={`w-full relative py-2.5 px-3 rounded-xl text-xs font-medium transition-all duration-200 border flex items-center justify-center gap-2 ${aspectRatio === ASPECT_RATIO_ORIGINAL
                ...
                {ASPECT_RATIOS.map(ratio => {
                  ...
                  return (
                    <button
                      ...
                      className={`group relative py-2 px-1 rounded-xl text-[10px] font-medium transition-all duration-200 border ${aspectRatio === ratio.value
                    ...
                    >
                      ...
                      <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:flex flex-col items-center bg-popover text-popover-foreground border border-border/50 p-2 rounded-xl shadow-xl z-[100] pointer-events-none scale-0 opacity-0 group-hover:scale-100 group-hover:opacity-100 transition-all origin-bottom duration-200">
                        ...
                        <span className="text-[10px] whitespace-nowrap opacity-70">{tr(`params.aspectRatio.option.${ratio.value}.desc`)}</span>
                      </div>
                    </button>
                  );
                })}
          ...
          {supportedParams.includes('imageSize') && (
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-muted-foreground/70 uppercase tracking-widest">{tr('create.resolutionLabel')}</label>
              <div className="relative">
                ...
                >
                  <option value="1K">{tr('params.imageSize.option.1K')}</option>
                  <option value="2K">{tr('params.imageSize.option.2K')}</option>
                  <option value="4K">{tr('params.imageSize.option.4K')}</option>
                </select>
          ...
          {/* Return Type */}
          <div className="space-y-2">
            <label className="text-[10px] font-bold text-muted-foreground/70 uppercase tracking-widest">{tr('create.returnTypeLabel')}</label>
            <div className="relative">
              <select
                value={returnType}
          ...
          <div className="space-y-2">
            ...
            >
              {refImages.map((img, index) => (
                <div
                  key={index}
                  className={`relative aspect-square rounded-lg overflow-hidden bg-secondary/20 group cursor-pointer ${index === newlyAddedRefIndex ? 'ring-2 ring-primary animate-pulse' : ''
                    }`}
...
function CopyPromptButton({ prompt, tr }: { prompt: string; tr: (key: string) => string }) {
  ...
  return (
    <button
      onClick={handleCopy}
      className={`group/copyprompt relative p-1.5 rounded-md transition-all ${copySuccess
          ? 'bg-green-500/10 text-green-500'
          : 'text-muted-foreground hover:bg-secondary hover:text-foreground'
        }`}
    >
