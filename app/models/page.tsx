  ...
  return (
    <div className="space-y-12 max-w-5xl mx-auto pb-20">
        ...
        {providers.length === 0 && !loading && (
          <div className="p-8 bg-card border border-dashed border-border rounded-2xl text-center">
            ...
            <p className="text-sm text-muted-foreground mb-4 max-w-sm mx-auto">
              {t('models.providers.emptyDesc') || '添加一个服务商以开始使用 AI 图像生成功能'}
            </p>
            <button
              onClick={() => setAddProviderModalOpen(true)}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {providers.map(p => {
            ...
            return (
              <div
                key={p.id}
                className={`p-5 rounded-2xl bg-card border group hover:shadow-lg transition-all ${needsApiKey
                    ...
                    <div className="space-y-2 max-h-48 overflow-y-auto">
                      {discoveredServices.map((service, idx) => (
                        <button
                          ...
                          className={`w-full flex items-center justify-between p-3 rounded-lg border transition-all ${selectedService?.url === service.url
      ...
      <AddProviderModal
        ...
        onOpenCustomModal={openNewProvider}
      />
    </div>
  );
}
